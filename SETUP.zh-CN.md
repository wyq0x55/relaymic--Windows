...
> 访问接收端的 `7420` 端口。接收端现在不监听任何端口，两端都主动连到你部署的控制面。
> 当前做法见 [`docs/signaling.md`](docs/signaling.md)。本文中音频设备相关的部分
> （BlackHole / VB-CABLE、编译、排错）仍然有效。
...
5. **语音输入的快捷键两边要设成不一样的**：本地和远程 Mac 如果都用同一个键触发听写，
   按一下两边一起响应，会冲突。让用户把远程那台改成别的键
6. **远程软件的声音传输要关掉**：UU 远程之类会把远程 Mac 的输入声音也传回本地，
   用户自己的声音会从本地喇叭放出来。在远程软件的设置里关掉声音传输就好
7. **延迟大概是多少**：网络往返 + 150ms 缓冲，接近一通电话。适合说话、听写、开会，
   **不适合录音时监听自己的声音**
8. **隐私**：音频走 WebRTC 加密的点对点连接，能直连时完全不经过第三方；作者没有
